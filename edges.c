	...
	while (1) {
		...
		if (iterator == 42) {
			...
			for (int i = 0; i < 10; i++) {
				...
				if (edges_log[i] & RIGHT_TOP) {
					printf("r%d", edges_log[i] & WITHOUT_TAGS);
				} else if (edges_log[i] & BOTTOM_TOP ) {
					printf("b%d", edges_log[i] & WITHOUT_TAGS);
				} else if (edges_log[i] & LEFT_TOP ) {
					printf("l%d", edges_log[i] & WITHOUT_TAGS);
				} else {
					printf("n%d", edges_log[i] & WITHOUT_TAGS);
				}
