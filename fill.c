...
unsigned char get_almost_fitting_pieces(unsigned int *buffer, const unsigned char top, const unsigned char right, const unsigned char bottom, const unsigned char left);
...
int get_color_number(char letter);
...
char missed_edge[256];
int missed_edges_count;
...
#include "precomputed-options-array.c"
    ...
    int init_position = atoi(argv[1]);

    char piece_number_string[4];
    char piece_colors_string[5];
    int piece_number;
    for (int i = 0; i < 256; i++) {

        piece_number_string[0] = input_string[1102+(i*3)];
        piece_number_string[1] = input_string[1102+(i*3)+1];
        piece_number_string[2] = input_string[1102+(i*3)+2];
        piece_number_string[3] = '\0';
        piece_number = atoi(piece_number_string);
        //printf("number %d \n", num);
        piece_colors_string[0] = input_string[64+(i*4)];
        piece_colors_string[1] = input_string[64+(i*4)+1];
        piece_colors_string[2] = input_string[64+(i*4)+2];
        piece_colors_string[3] = input_string[64+(i*4)+3];
        piece_colors_string[4] = '\0';
        //printf("colors %s \n", piece_colors_string);
        //printf("%d \n", get_color_number(piece_colors_string[0]));

        if ( piece_number > 0 ) {
            board[i]=&pieces[piece_number-1];
            pieces[piece_number-1].a = get_color_number(piece_colors_string[0]);
            pieces[piece_number-1].b = get_color_number(piece_colors_string[1]);
            pieces[piece_number-1].c = get_color_number(piece_colors_string[2]);
            pieces[piece_number-1].d = get_color_number(piece_colors_string[3]);
            pieces[piece_number-1].used = 1;
        }
	...
	while (1) {
		...
		if (iterator < init_position) {
                    printf("Break the filling. \n");
                    break;
                }
		...
		if (! fallback_flag) {
			...
			buffer_counts[current_buffer] = get_fitting_pieces(buffers[current_buffer], top, right,  bottom, left);
			    ...
			    buffer_counts[current_buffer] = get_almost_fitting_pieces(buffers[current_buffer], top, right,  bottom, left);
                            missed_edge[iterator] = 1;
                            //printf("No fitting piece at position %d, got %d almost fitting ones.\n", current, buffer_counts[current_buffer]);
                        } else {
                            missed_edge[iterator] = 0;
                            //printf("Got %d fitting pieces for position %d\n", buffer_counts[current_buffer], current);
                        }
		}
		...
		// Check if I have still pieces for special positions
                ...
                //printf("Missed edge count is %d", missed_edges_count);
                if (missed_edges_count > 10) force_fallback_flag = 1;
/*
		for (int q = 0; q < 12; q++) {
			int count;
		...
		if (buffer_counts[current_buffer] == 0 || force_fallback_flag) {
			...
			if (number_of_fallbacks < FALLBACKS*((max/60)*(max/60)+1)) {
				...
				fallback_flag = 1;
                                ...
                                missed_edges_count -= missed_edge[iterator];
...
// Allow one color not to fit
unsigned char get_almost_fitting_pieces(unsigned int *buffer, const unsigned char top, const unsigned char right, const unsigned char bottom, const unsigned char left) {
	int count = 0;

        if (top != 23) {
            for (int p = 0; p < options_lengths[23][right][bottom][left]; p++) {
                int i = options[23][right][bottom][left][p];
            ...
            for (int p = 0; p < options_lengths[top][23][bottom][left]; p++) {
                int i = options[top][23][bottom][left][p];
            ...
            for (int p = 0; p < options_lengths[top][right][23][left]; p++) {
                int i = options[top][right][23][left][p];
            ...
            for (int p = 0; p < options_lengths[top][right][bottom][23]; p++) {
                int i = options[top][right][bottom][23][p];
